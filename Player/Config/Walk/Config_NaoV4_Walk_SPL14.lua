...
walk.PIDX = {0,0,-0.08}
walk.filterX = {0.2, 1.5*math.pi/180,8*math.pi/180}
walk.PIDVelX = 60*math.pi/180




walk.PIDY = {0,0,-0.025}
walk.filterY = {0.5, 1.5*math.pi/180,8*math.pi/180}
walk.PIDVelY = 30*math.pi/180
...
walk.velLimitX={-.04,.04};
walk.supportModYInitial = -0.02

--[[
--Just for pocket!
if robotName=="pockets" then 
  walk.velLimitX={-.03,.04};  
  walk.velLimitY={-.02,.02};
  walk.velLimitA={-.4,.4};
  walk.velDelta={0.01,0.02,0.15} 
end
--]]
--[[
if robotName=="ticktock" then
walk.velLimitX={-0.02,0.02};
walk.velLimitY={-0.02,0.02};
walk.velDelta={0.01,0.02,0.15};
end
--]]
--[[
if robotName=="tink" then
  walk.velLimitX={-.02,.02};
  walk.velLimitY={-.02,.02};
  walk.velLimitA={-.4,.4};
  walk.velDelta={0.01,0.02,0.15}
end
...
--[[
if robotName=="hook" then 
  walk.bodyHeight = 0.27; 
walk.walkKickDef["FrontLeft"]={
  {0.30, 1, 0, 0.020 , {-0.02,0}, 0.6, {0.03,0,0} },
  {0.45, 2, 1, 0.035 , {-0.01,-0.01}, 0.5, {0.06,0,0}, {0.02,0,0} },
  {0.30, 1, 0, 0.020 , {0,0}, 0.5, {0.02,0,0} },
}
walk.walkKickDef["FrontRight"]={
  {0.30, 1, 1, 0.020 , {-0.02,0}, 0.4, {0.03,0,0} },
  {0.45, 2, 0, 0.035 , {-0.01,0.01}, 0.5,  {0.06,0,0}, {0.02,0,0} },
  {0.30, 1, 1, 0.020 , {0,0}, 0.5, {0.02,0,0} },
}
