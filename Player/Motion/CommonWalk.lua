...
--For webots op
walkKickVel = {0.06, 0.14};
walkKickSupportMod = {{0,0} , {0, 0}};
walkKickHeightFactor = 1.5;

--For webots nao
--[[
walkKickVel = {0.06, 0.14};
walkKickSupportMod = {{0, 0} , {-0.04, 0}};
walkKickHeightFactor = 3.0;
--]]
...
--Code to check walk-kick
    ...
    elseif walkKickRequest ==2 then -- Support step forward
      ...
      if supportLeg == 0 then        -- Left support
        uRight2 = pose_global({walkKickVel[1],-2*footY,0}, uLeft1);
      else        -- Right support
        uLeft2 = pose_global({walkKickVel[1],2*footY,0}, uRight1);
      end
    ...
    elseif walkKickRequest ==3 then -- Kicking step forward
      ...
      if supportLeg == 0 then        -- Left support
        uRight2 = pose_global({walkKickVel[2],-2*footY,0}, uLeft1);
      else        -- Right support
        uLeft2 = pose_global({walkKickVel[2],2*footY,0}, uRight1);
      end
    ...
    if supportLeg == 0 then
        uSupport = pose_global({supportX+supportMod[1], supportY+supportMod[2], 0}, uLeft);

        Body.set_lleg_hardness(.7*vector.ones(6));
        Body.set_rleg_hardness(.5*vector.ones(6));
    else
        -- Right support
        uSupport = pose_global({supportX+supportMod[1], -supportY-supportMod[2], 0}, uRight);
