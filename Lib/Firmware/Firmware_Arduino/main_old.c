...
int main(void)
{
  ...
  uart0_putstr("Switching to 1000000 baud...\r\n");
  _delay_ms(100);
  uart0_setbaud(1000000);
  uart0_putstr("Running at 1000000 baud\r\n");
  ...
  while (1) {
	...
	if (pktSerialInput.id != controlTable.id) {
	  ...
	  uart0_putstrn((char *)pktStatus, pktStatus->length+4);
	  PORTC |= _BV(PORTC5);
	}
      }
      c = uart0_getchar();
	...
	if (serialRs485Forwarded) {
	  ...
	  uart0_putstrn((char *)&pktRs485Input, pktRs485Input.length+4);
