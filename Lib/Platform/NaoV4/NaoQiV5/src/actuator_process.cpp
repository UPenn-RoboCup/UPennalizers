...
int actuator_process_init(boost::shared_ptr<ALBroker> pBroker) {
  AL::ALMemoryProxy* memoryAccess;

  try {
    dcm = new AL::DCMProxy(pBroker);
  } catch(ALError &e) {
