...
int actuator_process_init(ALPtr<ALBroker> pBroker) {
  tempBroker = pBroker;
  myfile.open ("/home/nao/UPennDev/Player/Lib/naoSensorLog.txt");    
  ...
  while (actuatorKey->key != NULL) {
    ...
    if (actuatorKey->names != NULL) {
      ...
      if (key.find("led") != std::string::npos) {
        ...
        //cmd.priority = 20;
        cmd.priority = 20;// sagar: it was 100
      }
      else {
        cmd.priority = 1;
      }
...
int actuator_process() {
  ...
  memoryProxy = boost::shared_ptr<AL::ALMemoryProxy>(new AL::ALMemoryProxy(tempBroker));
  const std::string actuatorKey("Device/SubDeviceList/LKneePitch/Position/Actuator/Value");
  const std::string sensorKey("Device/SubDeviceList/LKneePitch/Position/Sensor/Value");  
  ...
  myfile << time.tv_sec << " ";
  myfile << time.tv_nsec << " ";
  myfile << memoryProxy->getData(actuatorKey) << " ";
  myfile << memoryProxy->getData(sensorKey) << std::endl;
  ...
  for (unsigned int k = 0; k < commands.size(); k++) {
    ...
    cmd.dcmCommand[4][0] = dcm->getTime(0);
