--Darwin XOS Commmanager for Teen Size 2012
...
--[[
for i=1,12 do -- XOS only
posZero[i+6]=posZero[i+6]+legBias[i];
end
--]]
...
function entry()
  ...
  for i=1,12 do -- +6 b/c XOS has 4 arm things
...
-- Left arm
  for i=1,4 do
    actuator.bias[i+2]=armBias[i];
  end
-- Right arm
  for i=1,4 do
    actuator.bias[i+18]=armBias[i+4];
  end
...
--  print('mysens: ',Config.gyro.sensitivity)
    sensor.imuGyr[Config.gyro.rpy[i]] = 
    Config.gyro.sensitivity[i]*
