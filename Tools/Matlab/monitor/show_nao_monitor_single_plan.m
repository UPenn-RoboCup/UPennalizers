...
%This is new team monitor with logging and playback function
  ...
  global MONITOR LOGGER LUT COSTMAP;  
  ...
  function init(target_fps)
    ...
    MONITOR.distAxe = axes('Units','Normalized','position',[0.5 0.03 0.5 0.47]);

    MONITOR.costAxe = axes('Units','Normalized','position',[0 0.52 0.5 0.47]);
    MONITOR.cost2Axe = axes('Units','Normalized','position',[0.5 0.52 0.5 0.47]);
...
%}
  ...
  function update_monitor(robot)
    ...
    COSTMAP.update_cost(r_struct,r_mon);
    COSTMAP.update_value(r_struct,r_mon);    
    COSTMAP.calculate_trajectory(r_struct,r_mon);

    set(gcf,'CurrentAxes',MONITOR.mapAxe); 
%    plot_field(gca,MONITOR.fieldtype);
%    plot_robot( r_struct, r_mon,1.5,MONITOR.mapmode,'' );

    COSTMAP.draw2(r_struct,r_mon);    
    COSTMAP.draw_trajectory(r_struct,r_mon,'ko');
    if MONITOR.is_flip>0 view(180, 90);
    else view(0, 90); end
    

    set(gcf,'CurrentAxes',MONITOR.costAxe); 
    COSTMAP.draw(r_struct,r_mon);     
    COSTMAP.draw_trajectory(r_struct,r_mon,'ro');
    if MONITOR.is_flip>0 view(180, 90);
    else view(0, 90); end

    set(gcf,'CurrentAxes',MONITOR.cost2Axe); 
    %COSTMAP.draw3(r_struct,r_mon);
    plot_map(r_mon.robot.cost1);    
    plot_trajectory(r_mon,'ro');
    %plot_map(r_mon.robot.cost2);
    if MONITOR.is_flip>0 view(180, 90);
    else view(0, 90); end
  ...
  function plot_map(cost)
    ...
    surf([-4.5:0.1:4.5],[-3:0.1:3],cost2d','EdgeColor','none');%'
  ...
  function plot_trajectory(r,marker)
    ...
    plot3( traj_x*0.1-4.5,traj_y*0.1-3,110*ones(size(traj_x)),marker);
    hold off;
  end
