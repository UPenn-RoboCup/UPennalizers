...
%    set(gca, 'YTick', y_lim(1):0.1:y_lim(2));
    ...
    if isfield(mon_struct,'line') plot_line(mon_struct.line);end
    if isfield(mon_struct,'corner') plot_corner(mon_struct.corner);end
    if isfield(mon_struct,'circle') plot_circle(mon_struct.circle);end	
	if isfield(mon_struct,'free') plot_freespace(mon_struct.free);end
    hold off;
  ...
  function plot_line(line,scale)
    ...
    function plot_circle(circle,scale)
       if(circle.detect ==1)
          circle_x=circle.x;
          circle_y=circle.y;
          %display(circle_x);
          %display(circle_y);
          plot(circle_x,circle_y,'r*','MarkerSize',8);
       end
    end
