function h=show_monitor()
  ...
  function init(draw_team,target_fps)
	...
	'Units','Normalized', 'Position',[.60 .93 .10 .04],'Callback',@button7);
      ...
      MONITOR.hButton13=uicontrol('Style','pushbutton','String','Kidsize',...
	'Units','Normalized', 'Position',[.45 .2 .10 .04],'Callback',@button13);

      for i=1:5
        MONITOR.infoTexts(i)=uicontrol('Style','text',...
	'Units','Normalized', 'Position',[0.05,0.77-0.15*(i-1),0.05,0.13]);
      end

      for i=6:10
        MONITOR.infoTexts(i)=uicontrol('Style','text',...
	'Units','Normalized', 'Position',[0.92,0.77-0.15*(i-6),0.05,0.13]);
      end
	...
	'Units','Normalized', 'Position',[.02 .45 .07 .07],'Callback',@button11);
      ...
      MONITOR.hButton14=uicontrol('Style','pushbutton','String','Webots',...
	'Units','Normalized', 'Position',[.02 .38 .07 .07],'Callback',@button14);

      MONITOR.hInfoText=uicontrol('Style','text',...
	'Units','Normalized', 'Position',[.02 .25 .07 .13]);

      MONITOR.hButton13=uicontrol('Style','pushbutton','String','Kidsize',...
	'Units','Normalized', 'Position',[.02 .18 .07 .07],'Callback',@button13);

      MONITOR.hDebugTexttop=uicontrol('Style','text',...
	'Units','Normalized', 'Position',[.8 .47 .15 .44]);
    
      MONITOR.hDebugTextbtm=uicontrol('Style','text',...
	'Units','Normalized', 'Position',[.8 .01 .15 .44]);
  ...
  function update_single( robots, teamNumber, playerNumber )
    ...
    if MONITOR.enable1
      ...
      %top camera
      MONITOR.h1 = subplot(4,7,[3 4 10 11]);
      if MONITOR.is_webots    
        yuyv = robots{playerNumber,teamNumber}.get_yuyv(1);
        plot_yuyv(yuyv);
        if MONITOR.enable0
          plot_overlay(r_mon,2*MONITOR.enable1,1,1);
        end
      else
        if yuyv_type==1
          yuyv = robots{playerNumber,teamNumber}.get_yuyv(1);
  	      plot_yuyv(yuyv);
          plot_overlay(r_mon,1,1,1);
        elseif yuyv_type==2
          yuyv = robots{playerNumber,teamNumber}.get_yuyv2(1);
  	      plot_yuyv(yuyv);
          plot_overlay(r_mon,2,1,1);
        elseif yuyv_type==3
          yuyv = robots{playerNumber,teamNumber}.get_yuyv3(1);
  	      plot_yuyv(yuyv);
          plot_overlay(r_mon,4,1,1);
        end
      end

      %bottom camera
      MONITOR.h2 = subplot(4,7,[17 18 24 25]);
      %webots use non-subsampled label (2x size of yuyv)
      if MONITOR.is_webots
        yuyv = robots{playerNumber,teamNumber}.get_yuyv(2);
        plot_yuyv(yuyv);
        if MONITOR.enable0
%          plot_overlay(r_mon,2*MONITOR.enable1,1,2);
        end
      else
        if yuyv_type==1
          yuyv = robots{playerNumber,teamNumber}.get_yuyv(2);
          plot_yuyv(yuyv);
          plot_overlay(r_mon,1,1,2);       
        elseif yuyv_type==2
          yuyv = robots{playerNumber,teamNumber}.get_yuyv2(2);
          plot_yuyv(yuyv);
          plot_overlay(r_mon,2,1,2);
          
        elseif yuyv_type==3
          yuyv = robots{playerNumber,teamNumber}.get_yuyv3(2);
          plot_yuyv(yuyv);
          plot_overlay(r_mon,4,1,2);
        end
      ...
      if MONITOR.logging
        yuyvtop = robots{playerNumber,teamNumber}.get_yuyv(1);
        yuyvbtm = robots{playerNumber,teamNumber}.get_yuyv(2);
        LOGGER.log_data(yuyvtop + 0,yuyvbtm + 0,r_mon);
    ...
    if MONITOR.enable2==1
      %top camera
      MONITOR.h2 = subplot(4,7,[5 6 12 13]);
      labelA = robots{playerNumber,teamNumber}.get_labelA(1);
      plot_label(labelA);
      if MONITOR.enable0
        plot_overlay(r_mon,1,MONITOR.enable0,1);
      end
      
      %bottom camera
      MONITOR.h2 = subplot(4,7,[19 20 26 27]);
      labelA = robots{playerNumber,teamNumber}.get_labelA(2);
      plot_label(labelA);
      if MONITOR.enable0
        plot_overlay(r_mon,1,MONITOR.enable0,2);
      end
    ...
    elseif MONITOR.enable2==2
      %top camera
      MONITOR.h2 = subplot(4,7,[5 6 12 13]);
      labelB = robots{playerNumber,teamNumber}.get_labelB(1);
      plot_label(labelB);
      if MONITOR.enable0
        plot_overlay(r_mon,r_mon.camera.scaleBtop,MONITOR.enable0,1);
      end
      
      %bottom camera
      MONITOR.h2 = subplot(4,7,[19 20 26 27]);
      labelB = robots{playerNumber,teamNumber}.get_labelB(2);
      plot_label(labelB);
      if MONITOR.enable0
        plot_overlay(r_mon,r_mon.camera.scaleBbtm,MONITOR.enable0,2);
      end
    ...
    if MONITOR.enable3
      MONITOR.h3 = subplot(4,7,[1 2 8 9]);
      cla(MONITOR.h3);
...
%        plot_robot( r_struct, r_mon,2,3 );
    ...
    if MONITOR.enable4
      MONITOR.h4 = subplot(4,7,[15 16 22 23]);
      plot_surroundings( MONITOR.h4, r_mon );
    end
    
    if MONITOR.enable5
      if isfield(r_mon,'debugtop')
        set(MONITOR.hDebugTexttop,'String',r_mon.debugtop.message);
      end
      if isfield(r_mon,'debugbtm')
        set(MONITOR.hDebugTextbtm,'String',r_mon.debugbtm.message);
      end
    end
...
%        if MONITOR.deadcount(i) < 20 % ~2 sec interval until turning off
        if MONITOR.deadcount(i) < deadcount_threshold % ~5 sec interval until turning off
          ...
          updated = 0;
  	      if i<6 
            h1=subplot(5,7,(i-1)*7+1);
	          labelB = robot_team.get_labelB_wireless(i,1);
            plot_label(labelB);
	          plot_overlay_wireless(r_struct);
            h1=subplot(5,7,(i-1)*7+2);
	          labelB = robot_team.get_labelB_wireless(i,2);
            plot_label(labelB);
          else
            h1=subplot(5,7,(i-5)*7-1);
	          labelB = robot_team.get_labelB_wireless(i,1);
            plot_label(labelB);
	          plot_overlay_wireless(r_struct);
            h1=subplot(5,7,(i-5)*7);
	          labelB = robot_team.get_labelB_wireless(i,2);
            plot_label(labelB);
          end
...
%          set(MONITOR.infoTexts(i),'String',infostr2);
        ...
        elseif MONITOR.deadcount(i)==deadcount_threshold
          labelB1 = robot_team.get_labelB_wireless(i,1);
          labelB2 = robot_team.get_labelB_wireless(i,2);
  	      if i<6 
            h1=subplot(5,7,(i-1)*7+1);
            plot_label(labelB1*0);
            h1=subplot(5,7,(i-1)*7+2);
            plot_label(labelB2*0);
	        else
            h1=subplot(5,7,(i-5)*7-1);
            plot_label(labelB1*0);
            h1=subplot(5,7,(i-5)*7);
...
%          set(MONITOR.infoTexts(i),'String',infostr);
  ...
  function button5(varargin)
    ...
    if MONITOR.enable5 set(MONITOR.hButton5,'String', 'DEBUG ON');
      ...
      set(MONITOR.hDebugTexttop,'String','');
    end
  end
  ...
  function button14(varargin)
  
    MONITOR.is_webots = 1-MONITOR.is_webots;
    if MONITOR.is_webots==1
      set(MONITOR.hButton14, 'String', 'Webots On');
    else
      set(MONITOR.hButton14, 'String', 'Webots Off');
    end
