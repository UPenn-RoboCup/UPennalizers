...
%This is new team monitor with logging and playback function
  ...
  h.fieldtype=2; %0,1,2 for SPL/Kid/Teen
  ...
  function init(draw_team,target_fps)
	...
	'Units','Normalized', 'Position',[.60 .97 .10 .03],'Callback',@button2);
      ...
      MONITOR.hButton3=uicontrol('Style','pushbutton','String','Grasp',...
	'Units','Normalized', 'Position',[.70 .97 .10 .03],'Callback',@button3);
  ...
  function log_team_wireless(robot_team)
	...
	deadcount_threshold = 50;
        if MONITOR.deadcount(i)<deadcount_threshold
          ...
          TEAM_LOG.log_labelBT{TEAM_LOG.count}{i}=...
		        robot_team.get_labelB_wireless(i,1);
          TEAM_LOG.log_labelBB{TEAM_LOG.count}{i}=...
            robot_team.get_labelB_wireless(i,2);            
        end
      end
    end
  end
...
%        plot_overlay_wireless(r_struct);
  ...
  function button3(varargin)
    ...
    elseif MONITOR.fieldtype==2 set(MONITOR.hButton3,'String', 'Grasp');
