function plot_overlay(r_mon,scale,drawlevel, cidx)
%This function plots overlaid vision information
%Over the camera yuyv feed or labeled images
overlay_level=0;

%scale should be calculated from scaleA and scaleB!!!!!



if( ~isempty(r_mon) )
    if cidx==1 % TOP CAMERA
      scale_mul = 1;
        if((r_mon.ball.detect==1) && (r_mon.ball.detectbtm == 0))
            hold on;
        ...
        if( r_mon.goal.detect == 1)
            ...
            if (~isempty(r_mon.goal.postStat1))
                plot_goalposts(r_mon.goal.postStat1,r_mon.goal.v1,rollAngle,scale,r_mon.camera.scaleBtop);
                if(r_mon.goal.type==3)
                    plot_goalposts(r_mon.goal.postStat2,r_mon.goal.v2,rollAngle,scale,r_mon.camera.scaleBtop);
                end
            end
            hold off;
        end
        if ((r_mon.line.detect == 1) && (r_mon.line.detectbtm == 0))
            hold on;
            plot_line(r_mon.line,scale*4)
            hold off;
        end
    else % BOTTOM CAMERA
      scale_mul = 4;
        if((r_mon.ball.detect==1) && (r_mon.ball.detectbtm == 1))
            hold on;
            plot_ball( r_mon.ball, scale);
            hold off;
        end

        if( r_mon.spot.detect == 1)
            hold on;
            plot_spot(r_mon.spot,scale)
            hold off;
        end
        if ((r_mon.line.detect == 1) && (r_mon.line.detectbtm == 1))
            hold on;
            plot_line(r_mon.line,scale*4)
            hold off;
        end
        if (r_mon.corner.detect == 1)
            hold on;
            plot_corner(r_mon.corner,scale*2)
            hold off;
        end

        %if (r_mon.circle.detect == 1 && cidx == 2)
        %    hold on;
        %    plot_circle(r_mon.circle,scale)
        %    hold off;
        %end


        if (r_mon.free.detect == 1 && cidx == 2)
            hold on;
...
%Subfunctions
    ...
    function plot_ball( ballStats, scale )
        ...
        plot( centroid(1), centroid(2),'b+','MarkerSize',8,'LineWidth',2) % changed to blue for easier seeing
        if( ~isnan(ballBox) )
            rectangle('Position', ballBox, 'Curvature',[1,1],'LineWidth',1.5)
    ...
    function plot_spot(spotStats,scale)
        if scale==1
          spotscale = 2;
        else
          spotscale = 1;
        end
        bl = spotStats.bboxB(1)*spotscale;
        br = spotStats.bboxB(2)*spotscale;
        bt = spotStats.bboxB(3)*spotscale;
        bb = spotStats.bboxB(4)*spotscale;
       
        plot([bl br],[(bb+bt)/2 (bb+bt)/2],'b','LineWidth',5);
        plot([(bl+br)/2 (bl+br)/2],[bt bb],'b','LineWidth',5);
    ...
    function plot_line(lineStats,scale)
        ...
        for i=1:nLines
            ...
            x1=endpoint(1)/scale*8;
            x2=endpoint(2)/scale*8;
            y1=endpoint(3)/scale*8;
            y2=endpoint(4)/scale*8;
            xMean=lineStats.xMean(i)/scale*8;
            yMean=lineStats.yMean(i)/scale*8;
            plot([x1 x2],[y1 y2],'y','LineWidth',5);
            plot(xMean,yMean,'b*','MarkerSize',8);
        end
    end
    ...
    function plot_circle(circleStats,scale)
          x = circleStats.x/scale*8;
          y = circleStats.y/scale*8;
          %display(x);
          %display(y);
