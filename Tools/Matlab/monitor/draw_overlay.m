%function draw_overlay(r_mon,scale,drawlevel)
function h = draw_overlay()
...
h.plot_ball = @plot_ball;
    function plot_ball(h_axes, ball, scale )
        if ball.detect == 1
            radius = (ball.axisMajor / 2) / scale;
            centroid = [ball.centroid{1} ball.centroid{2}] / scale;
            ballBox = [centroid(1)-radius centroid(2)-radius 2*radius 2*radius];
            % set 'NextPlot' property to 'replacechildren' so image handle won't
            %   be destroyed during plot
            set(h_axes, 'NextPlot', 'replacechildren');
            plot(h_axes, centroid(1), centroid(2),'b+','MarkerSize',8,'LineWidth',2); % changed to blue for easier seeing
            if( ~isnan(ballBox) )
                rectangle('Position', ballBox, 'Curvature',[1,1], 'Parent', h_axes,...
                    'LindWidth',1.5);
                %        strballpos = sprintf('%.2f %.2f',ballStats.x,ballStats.y);
                %        b_name=text(centroid(1),centroid(2)+radius, strballpos, 'Parent', h_axes);
